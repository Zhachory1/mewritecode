#!/usr/bin/env bash
#
# install-local.sh — build mewrite-code from source and link it as global
# `mewrite` / `mewrite-code` / `mewritecode` commands.
#
# Usage (from a checkout of this repo):
#   ./scripts/install-local.sh
#
# This installs deps, builds all packages, and `npm link`s the CLI so the
# `mewrite` command points at THIS checkout. Re-run after pulling changes:
# only `npm run build` is needed once linked, but re-running this is safe.
#
# To uninstall:  npm unlink -g @zhachory1/mewrite-code
#
set -euo pipefail
...
echo "==> npm link (registers 'mewrite', 'mewrite-code', and 'mewritecode' globally)"
cd packages/coding-agent
npm link

echo
bin="$(command -v mewrite || true)"
if [ -n "$bin" ]; then
	echo "Done. mewrite -> $bin"
	mewrite --version
