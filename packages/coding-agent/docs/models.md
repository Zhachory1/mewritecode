...
Add custom providers and models (Ollama, vLLM, LM Studio, proxies) via `~/.mewrite/agent/models.json`.
...
Some OpenAI-compatible servers do not understand the `developer` role used for reasoning-capable models. For those providers, set `compat.supportsDeveloperRole` to `false` so Me Write Code sends the system prompt as a `system` message instead. If the server also does not support `reasoning_effort`, set `compat.supportsReasoningEffort` to `false` too.
...
For `models.json`, shell commands are resolved at request time. Me Write Code intentionally does not apply built-in TTL, stale reuse, or recovery logic for arbitrary commands. Different commands need different caching and failure strategies, and Me Write Code cannot infer the right one.
...
| `reasoningEffortMap` | Map Me Write Code thinking levels to provider-specific `reasoning_effort` values |
