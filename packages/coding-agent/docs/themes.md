> Me Write Code can create themes. Ask it to build one for your setup.
...
Me Write Code loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.mewrite/agent/themes/*.json`
- Project: `.mewrite/themes/*.json`
- Packages: `themes/` directories or legacy `pi.themes` entries in `package.json` (manifest key still named `pi`)
...
On first run, Me Write Code detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.mewrite/agent/themes
vim ~/.mewrite/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/Zhachory1/mewritecode/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, Me Write Code reloads it automatically for immediate visual feedback.
...
Me Write Code uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, Me Write Code falls back to the nearest approximation.
