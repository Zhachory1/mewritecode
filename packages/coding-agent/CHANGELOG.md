...
- `caveman subagents` CLI (#59 stage 3) for explicit bidirectional install:
  ...
  - Copies, not symlinks. Skips files already at the destination (no `--force` — user-edited personas are never clobbered). Idempotent on re-run. Tool-name aliasing happens at LOAD time so files are copied verbatim and remain valid CC personas on disk ([#59](https://github.com/JuliusBrussee/caveman-code/issues/59)).
