...
- `mewrite agents` focus pane now shows a context-usage meter in its status line: a progress bar `███░░░ 20% 40k` (percent of context window + tokens used) plus the thinking level next to the model id, matching the interactive context meter. Fed by the daemon `usage` notification (see #176), so it live-updates on each turn; before the first usage event it shows just the model id, and post-compaction it renders `?` for unknown token counts ([#173](https://github.com/Zhachory1/mewritecode/issues/173)).
