...
- `mewrite agents` focus pane now shows the interactive context-window meter in its status line: `{percent}%/{tokens}` plus the thinking level next to the model id, matching the interactive footer. Fed by the daemon `usage` notification (see #176), so it live-updates on each turn; before the first usage event it shows just the model id, and post-compaction it renders `?` for unknown token counts, same as the footer ([#173](https://github.com/Zhachory1/mewritecode/issues/173)).
