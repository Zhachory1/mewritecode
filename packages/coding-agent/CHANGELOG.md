...
- Added `mewrite history`: a standalone TUI to browse and resume persisted local JSONL sessions. Select a session and press enter to see the exact `mewrite --resume <path>` command to copy and run. This is separate from the live daemon monitor (`mewrite agents`) and from `mewrite sessions` (which lists daemon-hosted sessions via the client). Read-only: no delete or rename, just browse and get the resume command ([#159](https://github.com/Zhachory1/mewritecode/issues/159)).
