...
- New brand logo: a pencil (code shaft) with "Me Write Code / me write less, me do more" to its right, replacing the ASCII wordmark. Shown on interactive startup and on the `mewrite agents` launch screen (shared renderer). Terminal-safe box-drawing, no image component ([#185](https://github.com/Zhachory1/mewritecode/issues/185)).
...
- Release workflow no longer pushes directly to protected `main`: the Homebrew formula bump now lands via an auto-mergeable PR and nightly bench results are committed to a dedicated `bench-results` branch ([#124](https://github.com/Zhachory1/mewritecode/pull/124), [#125](https://github.com/Zhachory1/mewritecode/pull/125))
...
- Added a skills dropdown to the served web UI composer: typing `/` at the start of a line opens a fuzzy-filtered list of skills discovered under the session cwd. Arrow keys or hover navigate; Enter/Tab/click inserts `use skill: <name> — <description>` at the trigger position; Escape closes. Backed by a new `GET /v1/sessions/:id/skills` endpoint. Slash-command execution is a separate follow-up ([#111](https://github.com/Zhachory1/mewritecode/issues/111)).
...
- Added a text-size control to the served web UI header (numeric px input, clamped 10-24) so the chat, editor, composer, file tree, and skill menu scale together while UI chrome stays fixed; the selected size persists across reloads in `localStorage` ([#101](https://github.com/Zhachory1/mewritecode/issues/101)).
...
- Extracted cohesive interactive slash-command routing from `InteractiveMode` so built-in command conditions and callbacks are centralized and directly tested ([#36](https://github.com/Zhachory1/mewritecode/issues/36)).
...
- Added downstream distribution metadata support for branded wrappers, including bootstrap package-dir overrides and exported config helpers ([#31](https://github.com/Zhachory1/mewritecode/issues/31)).
...
- `caveman subagents` CLI (#59 stage 3) for explicit bidirectional install:
  ...
  - `caveman subagents install --to claude [--dry-run]` — copy cave-bundled agents into `~/.claude/agents/` so Claude Code can use them too.
  - Copies, not symlinks. Skips files already at the destination (no `--force` — user-edited personas are never clobbered). Idempotent on re-run. Tool-name aliasing happens at LOAD time so files are copied verbatim and remain valid CC personas on disk ([#59](https://github.com/Zhachory1/mewritecode/issues/59)).
- Cross-platform agent discovery (#59 stage 2). Agents authored for Claude Code (`~/.claude/agents/*.md` and `<cwd>/.claude/agents/*.md`) can now be loaded into cave when opted in. **Default OFF** for privacy — reading from another tool's config dir is consent-bearing. Enable via the `CAVE_CROSS_PLATFORM_AGENTS=true` env var or by passing `crossPlatformDiscovery: true` to `loadAgentDefs`. V1 covers Claude Code only; Cursor / Codex / OpenCode deferred until their actual config dirs + frontmatter formats are verified ([#59](https://github.com/Zhachory1/mewritecode/issues/59)).
- Claude-Code-cased tool names in agent frontmatter are now aliased to cave canonical at parse time: `Read → read`, `Bash → bash`, `Edit → edit`, `Write → write`, `Grep → grep`, `LS → ls`, `Glob → grep + find` (one-to-many). A persona authored for Claude Code with `tools: Read, Bash, Edit` now gets `tools: [read, bash, edit]` after load — no manual rewrite required. The raw frontmatter list is preserved on disk; the alias rewrite is observable via a single per-persona compatibility diagnostic that lists every alias that fired. Output format and parameters may differ slightly from CC — see `CC_TOOL_ALIASES` in `tool-name-check.ts` for per-tool caveats ([#59](https://github.com/Zhachory1/mewritecode/issues/59)).
- Subagent extension is now bundled by default. `caveman` loads the subagent tool on startup with no install step required — invoke it via `Use scout to find auth code` or the dispatch tool. Sample agents (`scout`, `planner`, `worker`) ship in `packages/coding-agent/agents/` alongside the existing bundled six (`critic`, `editor`, `explore`, `implementer`, `reviewer`, `tester`). Power-user override path: drop a modified `index.ts` / `agents.ts` under `~/.cave/extensions/subagent/` (user scope) or `.cave/extensions/subagent/` (project scope) ([#59](https://github.com/Zhachory1/mewritecode/issues/59)).
- Subagent workflow prompts (`/implement`, `/scout-and-plan`, `/implement-and-review`) ship in `packages/coding-agent/prompts/` and load automatically from every cave session. New `getBundledPromptsDir()` config helper and a bundled-defaults scan path in the resource loader (lowest precedence — user/project entries override by name via `dedupePrompts`). Opted out by `--no-prompt-templates`, same as user/project discovery ([#60](https://github.com/Zhachory1/mewritecode/issues/60)).

### Changed

- `discoverAndLoadExtensions` accepts a new `{ skipBundled?: boolean }` options object so tests asserting exact extension counts can opt out of the bundled defaults. Mirrors `loadAgentDefs`'s `skipBundled` option ([#59](https://github.com/Zhachory1/mewritecode/issues/59)).
- Extracted the Tribal Signal (context-drift warnings) and Fire Starter (preemptive compaction) heuristics out of `InteractiveMode` into a pure `context-drift-widgets.ts` module. No behavior change; the evaluator now has direct unit-test coverage (18 tests). Part of the `interactive-mode.ts` / `agent-session.ts` god-object decomposition ([#16](https://github.com/Zhachory1/mewritecode/issues/16)).
...
- Sequential command orchestration with `/then`: chain commands across turns, e.g. `/writing-plans /then /goal implement until tests are green`. The head runs in the current turn; tail entries dispatch after each turn settles. New `/queue` built-in lists or clears the pending chain; Escape pulls queued entries back into the editor. `/then` is only treated as a separator when followed by another `/command` token, so prose like "first do X then do Y" is untouched, and `/then` inside code fences/backticks is preserved verbatim ([#5](https://github.com/Zhachory1/mewritecode/issues/5)).

### Fixed

- Added missing `@sinclair/typebox` runtime dependency. Fixes `ERR_MODULE_NOT_FOUND` on `caveman-code` startup when installed globally ([#6](https://github.com/Zhachory1/mewritecode/issues/6)).
- Fixed broken extensions-migration links in the hooks-folder warning — now point to `Zhachory1/mewritecode` instead of the renamed-away `caveman-cli` repo ([#8](https://github.com/Zhachory1/mewritecode/issues/8)).
- Self-update check now queries the correct GitHub repo (`caveman-code`) for releases.
- Regenerated model registry to include GPT 5.5 / GPT 5.5 Pro for the OpenAI Codex login provider ([#7](https://github.com/Zhachory1/mewritecode/issues/7)) and refreshed Antigravity model entries ([#9](https://github.com/Zhachory1/mewritecode/issues/9)).
