...
- `mewrite agents` now has a read-only transcript view: select an interactive `[i]` session and press enter to see its conversation, scroll with arrows, and `esc`/`q` to return to the list. Interactive transcripts are read from the local session JSONL (no daemon); hosted `[d]` sessions still hand off to the attach REPL ([#157](https://github.com/Zhachory1/mewritecode/issues/157)).
