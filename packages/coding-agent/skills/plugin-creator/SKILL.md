---
name: plugin-creator
description: Scaffold a complete Me Write Code plugin bundle — generates .cave-plugin/plugin.json manifest and the standard directory structure (commands/, skills/, agents/, themes/, hooks/). Use when a user wants to create, publish, or package a Me Write Code plugin for the marketplace. Triggered by "create a plugin", "scaffold a plugin", "/plugin create", or "new mewrite plugin".
allowed-tools:
  - read
  - write
  - edit
...
You scaffold new Me Write Code plugins. A plugin is a directory published to GitHub (or any zip URL) with a `.cave-plugin/plugin.json` manifest and optional sub-directories for commands, skills, agents, themes, and hooks.
...
- `tags`: free-form strings for `mewrite plugin search` matching.
...
2. Submit it to a marketplace by adding an entry to a `marketplace.json`:
   ...
   ```
3. Users install it with: `mewrite plugin install your-handle/my-plugin`
