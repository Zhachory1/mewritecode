...
!((e, t) => {
	...
	const i = {
		...
		D = {
			...
			punctuation: z,
			reflink: Z,
			reflinkSearch: r("reflink|nolink(?!\\()", "g").replace("reflink", Z).replace("nolink", v).getRegex(),
			tag: C,
			text: /^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<![`*_]|\b_|$)|[^ ](?= {2,}\n)))/,
			url: s,
		},
					...
					if (
						...
						for (; e; ) {
							...
							if (g.search(this.rules.other.nonSpaceChar) >= p || !c.trim()) o += "\n" + g.slice(p);
								...
								if (h) break;
								if (
									a.replace(this.rules.other.tabCharGlobal, "    ").search(this.rules.other.nonSpaceChar) >= 4
								)
									break;
								if (r.test(a)) break;
								if (i.test(a)) break;
		...
		table(e) {
			...
			if (n.length === s.length) {
				...
				for (let e = 0; e < n.length; e++)
					i.header.push({ text: n[e], tokens: this.lexer.inline(n[e]), header: !0, align: i.align[e] });
				for (const e of r)
					i.rows.push(
	...
	class W {
		...
		constructor(t) {
			...
			this.options.pedantic
				...
				: this.options.gfm && ((n.block = j.gfm), this.options.breaks ? (n.inline = N.breaks) : (n.inline = N.gfm)),
	...
	(ie.options = ie.setOptions = (e) => (re.setOptions(e), (ie.defaults = re.defaults), n(ie.defaults), ie)),
		(ie.getDefaults = t),
		(ie.defaults = e.defaults),
		(ie.use = (...e) => (re.use(...e), (ie.defaults = re.defaults), n(ie.defaults), ie)),
		(ie.walkTokens = (e, t) => re.walkTokens(e, t)),
