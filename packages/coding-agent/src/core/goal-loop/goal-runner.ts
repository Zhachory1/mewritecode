/**
 * Goal-loop driver. Spawns `mewrite -p --mode json` per iteration with the
...
export const runGoal = async (opts: RunGoalOptions): Promise<RunGoalResult> => {
	...
	while (true) {
		...
		if (consecutiveNoProgress >= frontmatter.max_no_progress) {
			return finish(
				"paused",
				`no progress for ${consecutiveNoProgress} iterations — run \`mewrite goal resume ${opts.id} --force\` to continue`,
				ledger,
			);
		}
