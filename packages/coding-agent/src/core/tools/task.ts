	...
	return {
		...
		async execute(toolCallId, params: TaskToolInput, signal) {
			...
			if (hasSingle) {
				...
				const text =
					r.exitCode === 0
						? `${artifactHint}${r.output || "(no output)"}`
						: `${artifactHint}Subagent failed: ${r.error || r.output || "(no output)"}`;
				return {
			...
			if (hasParallel) {
				...
				const summary = results.map((r) => {
					const preview = r.output.slice(0, RESULT_PREVIEW_CHARS);
					const suffix =
						r.truncated && r.fullOutputPath ? ` … (truncated; fullOutputPath: ${r.fullOutputPath})` : "";
					return `[${r.agent}] ${r.exitCode === 0 ? "OK" : "FAIL"}: ${preview}${suffix}`;
				});
				return {
					content: [
						{
			...
			return {
				content: [{ type: "text" as const, text: `${artifactHint}${last?.output || "(no output)"}` }],
				details: { mode: "chain" as const, results },
			};
		},
