...
```bash
npm install @zhachory1/mewrite-mom
```
...
- run interactive coding agent session: `npx cave`
- enter `/login` command
...
Update mom anytime with `npm install -g @zhachory1/mewrite-mom`. This only updates the Node.js app on your host. Anything mom installed inside the Docker container remains unchanged.
